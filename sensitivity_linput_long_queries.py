...
plt.savefig('manuscript_figures/updated_figures/figure2_energy_reasoning_sens.svg', format='svg', dpi=300, bbox_inches='tight')
plt.savefig('manuscript_figures/updated_figures/figure2_energy_reasoning_sens.png', format='png', dpi=300, bbox_inches='tight')
...
# Legacy single-figure code (keeping for compatibility)
# Simulation settings for single figure
n_runs = 10000
median_tokens = 5000   # median tokens query length
fixed_input_length = 5000  # Fixed input length for predictions
# Calculate lambda parameter for exponential distribution to achieve desired median
lambda_param = np.log(2) / median_tokens  # For exponential, median = ln(2)/λ

# Define ranges and values
def get_node_power(model_name):
    return 14.1 if model_name == 'DeepSeek-R1' else 11.3
...
plt.savefig('manuscript_figures/figure1_per_query_energy_consumption_tts.svg', format='svg', dpi=300, bbox_inches='tight')
plt.savefig('manuscript_figures/figure1_per_query_energy_consumption_tts.png', format='png', dpi=300, bbox_inches='tight')
...
print(f"Hardware multiplier range: {hardware_multiplier.min():.2f} - {hardware_multiplier.max():.2f}")
print(f"Model multiplier range: {algorithm_multiplier.min():.2f} - {algorithm_multiplier.max():.2f}")
print(f"Serving multiplier range: {improved_multiplier.min():.2f} - {improved_multiplier.max():.2f}")
...
plt.savefig('manuscript_figures/figure2_energy_improvement_pathways_tts.svg', format='svg', dpi=300, bbox_inches='tight')
plt.savefig('manuscript_figures/figure2_energy_improvement_pathways_tts.png', format='png', dpi=300, bbox_inches='tight')
