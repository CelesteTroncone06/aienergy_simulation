...
for model_name in all_tps_models.keys():
   ...
   # ----------------------------------------------------------
   # Store the PUE-independent energy term
   # ----------------------------------------------------------
    base_energy = (
        model_node_power_array
        * model_pu
        * model_token_lengths
        / model_tokens_per_sec
        )

    all_model_base_energy[model_name] = base_energy

# Calculate baseline energies exactly as before
    model_energies = (model_pue * base_energy / 3600) * 1000
...
print("\nRunning PUE sensitivity analysis...")
...
for fixed_pue in pue_values:

    all_model_energies_pue = {}

    # Recreate energies using identical Monte Carlo samples
    for model_name, base_energy in all_model_base_energy.items():
        ...
        all_model_energies_pue[model_name] = energies

    # Rename models exactly as Figure 1
    if 'DeepSeek-R1' in all_model_energies_pue:
        all_model_energies_pue['DeepSeek-R1 671B'] = all_model_energies_pue.pop('DeepSeek-R1')

    if 'Llama-3.1 Nemotron Ultra 253B' in all_model_energies_pue:
        all_model_energies_pue['Llama-3.1 Nemotron\nUltra 253B'] = all_model_energies_pue.pop(
            'Llama-3.1 Nemotron Ultra 253B'
        )

    # Same top-3 calculation
    model_medians = {}

    for model_name in model_order:
        if model_name in all_model_energies_pue:
            model_medians[model_name] = np.median(all_model_energies_pue[model_name])
    ...
    for model_name in top_3_models:
        ...
        energies = all_model_energies_pue[model_name]
        ...
        # Stage 2 filtering
        if additional_filtering:
            p10, p90 = np.percentile(stage1,[10,90])
            stage1 = stage1[
                (stage1 >= p10) &
                (stage1 <= p90)
            ]
    ...
    median_energy_pue.append(np.median(mixed))
    p5_energy_pue.append(np.percentile(mixed,5))
    p95_energy_pue.append(np.percentile(mixed,95))
...
with plt.rc_context({
    ...
    plt.title("Sensitivity of Query Energy to PUE")
    ...
    plt.savefig(
        'manuscript_figures/updated_figures/pue_sensitivity.svg',
        format='svg',
        dpi=300,
        bbox_inches='tight'
    )
    ...
    plt.savefig(
        'manuscript_figures/updated_figures/pue_sensitivity.png',
        format='png',
        dpi=300,
        bbox_inches='tight'
    )
