...
# --- PU–TPS coupling settings ---
COUPLE_PU_TO_TPS = True        # set False to revert to independent PU sampling
PU_BASE = 0.24                  # PU at zero throughput
PU_SPAN = 0.56                  # additional PU gained as TPS -> TPS_max (so PU_max = 0.24 + 0.66 = 0.90)
PU_JITTER = 0.0                 # optional fractional jitter around coupled PU (e.g., 0.05 for ±5% lognormal noise)
...
for model_name in all_tps_models.keys():
    ...
    if COUPLE_PU_TO_TPS:
        # Coupled PU: PU = 0.24 + 0.66 * (TPS / TPS_max), clipped to [0.24, 0.90]
        # Get TPS_max from your fitted dataset (fallback to observed max of current batch if missing)
        tps_max = max_tps_values.get(model_name, np.nanmax(model_tokens_per_sec))
        if not np.isfinite(tps_max) or tps_max <= 0:
            # Safety fallback to avoid division issues
            tps_max = np.nanmax(model_tokens_per_sec[model_tokens_per_sec > 0])

        tps_norm = np.clip(model_tokens_per_sec / tps_max, 0.0, 1.0)
        model_pu = PU_BASE + PU_SPAN * tps_norm  # vectorized

        # Optional jitter around the coupled PU (lognormal with median 1.0)
        if PU_JITTER and PU_JITTER > 0:
            jitter_sigma = np.log(1.0 + PU_JITTER)
            jitter = np.random.lognormal(mean=0.0, sigma=jitter_sigma, size=n_runs)
            model_pu = np.clip(model_pu * jitter, PU_BASE, PU_BASE + PU_SPAN)
    else:
        # Original independent PU sampling
        model_pu = np.random.lognormal(mu_pu, sigma_pu, n_runs)
        # Keep PU in [0.0, 1.0] just in case of tails (optional)
        model_pu = np.clip(model_pu, 0.0, 1.0)
    
    # Calculate base energies for this model (vectorized for speed)
    # time per query in seconds = L_eff / TPS; here L_eff ~ output length
    time_seconds = model_token_lengths / model_tokens_per_sec
    # (kW * s) / 3600 * 1000 -> Wh simplified: /3.6 since kW*s to Wh = /3600*1000
    model_energies = (model_pue * (model_node_power_array * model_pu * time_seconds) / 3600.0) * 1000.0
    
    all_model_energies[model_name] = model_energies
    print(f"  Generated {n_runs} energy samples")

# Print PU policy used
print(f"\nPU policy: {'Coupled to TPS (PU = 0.24 + 0.66 * TPS/TPS_max)'+(' with jitter' if PU_JITTER>0 else '') if COUPLE_PU_TO_TPS else 'Independent lognormal sampling'}")
    ...
    if model_name in all_model_energies:
        ...
        if not np.isnan(energies).all():
            ...
            legend_elements.append(plt.Line2D([0], [0], color=color_dict[model_name], 
                                 label=f'{model_name}: {p50:.2f} Wh (IQR:{p25:.2f}-{p75:.2f})', 
                                 linewidth=3))
...
# plt.savefig('manuscript_figures/figure1_per_query_energy_consumption_tts_PUsens.svg', format='svg', dpi=300, bbox_inches='tight')
# plt.savefig('manuscript_figures/figure1_per_query_energy_consumption_tts_PUsens.png', format='png', dpi=300, bbox_inches='tight')
...
plt.savefig('manuscript_figures/updated_figures/figure2_PUsens_reasoning.svg', format='svg', dpi=300, bbox_inches='tight')
plt.savefig('manuscript_figures/updated_figures/figure2_PUsens_reasoning.png', format='png', dpi=300, bbox_inches='tight')
...
# plt.savefig('manuscript_figures/figure3_token_length_distribution_tts.svg', format='svg', dpi=300, bbox_inches='tight')
# plt.savefig('manuscript_figures/figure3_token_length_distribution_tts.png', format='png', dpi=300, bbox_inches='tight')
