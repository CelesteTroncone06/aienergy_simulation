#%%
"""
PARAMETER ALIGNMENT WITH estimate_per_mode_regression_reasoning.py:
...
3. IMPROVEMENT MULTIPLIERS:
   ...
   - Model Routing / Token Budget: lognorm(1.5, 5) (was lognorm(1.5, 2))

4. STYLING:
   - Font sizes aligned with reference file
   - Figure size: (10, 8) for consistency
   - Seaborn font scale: 2.5
   - All legend and tick font sizes updated to match

5. FUZZ FACTOR:
   - Using 1.10*1.076 = 1.1836 from 1B queries file
"""
import numpy as np
...
mu_routing, sigma_routing = lognorm_params(1.5, 5)  # Model Routing / Token Budget multiplier
mu_algorithm, sigma_algorithm = lognorm_params(1.5, 10)  # Algorithm/Model multiplier
mu_improved, sigma_improved = lognorm_params(1.5, 3.5)  # Improved serving multiplier
...
routing_multiplier = np.random.lognormal(mu_routing, sigma_routing, n_mixed_samples)
...
routing_energies = mixed_energies / routing_multiplier
algorithm_energies = mixed_energies / algorithm_multiplier
improved_energies = mixed_energies / improved_multiplier

print(f"Applied improvement multipliers to {n_mixed_samples:,} baseline samples")
print(f"Hardware multiplier range: {hardware_multiplier.min():.2f} - {hardware_multiplier.max():.2f}")
print(f"Model Routing / Token Budget multiplier range: {routing_multiplier.min():.2f} - {routing_multiplier.max():.2f}")
...
plot_data_all = pd.concat([
    ...
    prepare_improvement_data(routing_energies, 'Model Routing / Token Budget'),
...
colors = {
    'Baseline': '#2ecc71',
    'Hardware': '#3498db',
    'Model Routing / Token Budget': '#9b59b6',
    'Algorithm/Model': '#e67e22',
    'Improved\nServing': '#e74c3c'
}
...
distribution_names = ['Baseline', 'Hardware', 'Model Routing / Token Budget', 'Algorithm/Model', 'Improved\nServing']
energies_list = [mixed_energies, hardware_energies, routing_energies, algorithm_energies, improved_energies]
...
for category, energies in [
    ...
    ('Model Routing / Token Budget', routing_energies),
...
plt.axvline(median_actual, color='orange', linestyle='--', linewidth=2, 
           label=f'Median: {median_actual:.0f} tokens')
...
# Multiply by 1.1836 to account for orchestration, backup, etc.
fuzz_factor = 1.1836*1.12
...
# Customize the plot
plt.title('Estimated Total Energy Consumption\nfor 1 Billion Reasoning Queries', fontsize=14, pad=20)
...
improvement_pct = (1 - improved_median/baseline_median) * 100
print(f"\nSUMMARY COMPARISON:")
print(f"Baseline: {baseline_median * fuzz_factor:.2f} GWh")
print(f"Improved scenario: {improved_median * fuzz_factor:.2f} GWh")
print(f"Improvement: {improvement_pct:.1f}% reduction ({baseline_median * fuzz_factor:.2f} → {improved_median * fuzz_factor:.2f} GWh)")
...
# Add column model_type "Reasoning"
df['model_type'] = 'Reasoning'

df.to_csv('1B_queries_energy_consumption_reasoning.csv', index=False)

print(f"\nResults saved to '1B_queries_energy_consumption_reasoning.csv'")
